...
$userData += @(
    ...
    '      if [ -e /var/log/syslog ]; then cp -f /var/log/syslog /boot/firmware/cloud-init-debug/syslog.log; else journalctl -b --no-pager > /boot/firmware/cloud-init-debug/syslog.log 2>&1 || true; fi'
    '      iw reg get > /boot/firmware/cloud-init-debug/iw-reg.txt 2>&1 || true'
    '      iw dev wlan0 info > /boot/firmware/cloud-init-debug/iw-info.txt 2>&1 || true'
    '      iw dev wlan0 link > /boot/firmware/cloud-init-debug/iw-link.txt 2>&1 || true'
    '      dmesg | grep -Ei ''brcmfmac|cfg80211|wlan0|firmware|regulatory'' > /boot/firmware/cloud-init-debug/wlan-kernel.log 2>&1 || true'
...
$userData += @(
    ...
    '  - if [ -e /dev/ttyAMA10 ]; then systemctl enable serial-getty@ttyAMA10.service; systemctl restart serial-getty@ttyAMA10.service; fi'
    '  - if [ -e /dev/ttyS0 ]; then systemctl enable serial-getty@ttyS0.service; systemctl restart serial-getty@ttyS0.service; fi'
